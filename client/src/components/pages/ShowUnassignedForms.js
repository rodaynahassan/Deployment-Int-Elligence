    ...
    componentDidMount(){
      ...
      tabRow=()=>{
        return this.state.ShowUnassignedForms.map((ShowUnassignedForm, i)=>{
            return <UnassignedComponent ShowUnassignedForm={ShowUnassignedForm} setFormId={this.props.setFormId} key={i} delete={this.delete} />;
        });
      }
      render(){
        return (
          <div style={{ paddingLeft:'60px',justifyItems:"center"}}>
          <h2 align="center"><Badge variant="dark">Your Unassigned forms</Badge></h2>
          <Navbar bg='dark'> 
