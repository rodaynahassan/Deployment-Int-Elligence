...
class assignToReviewer extends Component 
{
    state = {
      ...
      componentDidMount(){
        ...
        axios.get('http://localhost:5000/routes/api/userDynamicForms/getReviewerPossiblePicks',{headers: { "Authorization": localStorage.getItem('jwtToken') }})
        .then(res => {
        ...
        getAttributes =()=>{
          ...
          return(
            ...
            flipOnHover={false} 
             ...
             <Button type="button" variant="ali" onClick={()=>(this.AssignReviewer(Form._id), alert('This Case is assigned to YOU!!'))}class="btn btn-info"><h6 style={{color:"#64b9e0" }}>Pick Case</h6></Button>
              </div>
              </FrontSide>
            <BackSide
