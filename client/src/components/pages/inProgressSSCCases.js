...
class InProgressSSCCases extends Component {
    ...
    componentDidMount(){
      ...
      axios.get('http://localhost:5000/routes/api/users/getInProgressSSCCases/',{headers: { "Authorization": localStorage.getItem('jwtToken') }})
      .then(res => {
      ...
      tabRow=()=>{
        return this.state.inProgressSSCCases.map((inProgressSSCCase, i)=>{
            return <InProgressSSCCasesComponents inProgressSSCCase={inProgressSSCCase}  setFormId={this.props.setFormId} key={i} />;
        });
      }
      render(){
        return (
