...
const functions = {
	...
	UpdateFormInUser: async (UserId, FormId) => {
		// update a form in a certain user
		return await axios({
			method: 'put',
			url: '/routes/api/users/' + UserId + '/' + FormId,
			data: {
	...
	DeleteUser: async (deleteID) => {
		const user = await axios.delete('/routes/api/users/' + deleteID);
