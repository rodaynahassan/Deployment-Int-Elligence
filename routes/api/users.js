...
//Delete User without authorization for testing
router.delete("/deleteU/:id",async (req, res) => {
      try {
        const id = req.params.id;
        var SpecificUser = await userController.search("_id", id);
        if (SpecificUser.error) return res.json({ msg: "This user doesnt exist" , error : SpecificUser.error});
        for (i = 0; i < SpecificUser.forms.length; i++) {
         ...
         return res.json({ msg: "User was deleted successfully", data: deletedUser });
        }
      } catch (error) {
        console.log(error);
      }
