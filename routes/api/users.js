...
router.get('/getInProgressSPCCases/', passport.authenticate('jwt', { session: false }), async (req, res) => {
	...
	if (req.user.userType === 'Investor' || req.user.userType === 'Lawyer') {
		...
		for (i = 0; i < userForms.length; i++) {
			if (
				(userForms[i].type === 'SPCForm' && userForms[i].status === 'In progress Lawyer') ||
...
router.get('/getInProgressSSCCases/', passport.authenticate('jwt', { session: false }), async (req, res) => {
	...
	if (req.user.userType === 'Investor' || req.user.userType === 'Lawyer') {
		...
		for (i = 0; i < userForms.length; i++) {
			if (
				(userForms[i].type === 'SSCForm' && userForms[i].status === 'In progress Lawyer') ||
				userForms[i].status === 'In progress Reviewer'
			)
				inprogressForms.push(userForms[i]);
		}
		...
		if (user.userType === 'Lawyer') {
			for (i = 0; i < userForms.length; i++) {
				if (
					...
					(userForms[i].status === 'In progress Lawyer' || userForms[i].status === 'Lawyer accepted')
				) {
...
router.get('/getApprovedCompanies', passport.authenticate('jwt', { session: false }), async (req, res) => {
	...
	if (req.user.userType === 'Investor') {
		...
		if (user.userType === 'Investor') {
			...
			var approvedForms = [];
			for (i = 0; i < userForms.length; i++) {
				if (userForms[i].status === 'Approved') approvedForms.push(userForms[i]);
			}
